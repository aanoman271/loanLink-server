...
async function connectDb() {
  ...
  app.get("/approvedLoan-count", async (req, res) => {
    try {
      const paid = await loanApplicationCollection.countDocuments({ status: "Approved" })
      const unpaid = await loanApplicationCollection.countDocuments({ status: "pending" })
      const approvedLoan = await loanApplicationCollection.find({ status: "Approved" }).toArray()
      const sumOfAmount = approvedLoan.reduce((sum, item) => sum + (Number(item.loanAmount) || 0), 0)
